...
final class ProjetRepository implements ProjetRepositoryInterface
{
    ...
    public function findById(ProjetId $id): ?Projet
    {
        $stmt = $this->pdo->prepare(
            'SELECT id_projet, titre_projet, texte_projet, date_projet, url_projet, urlGitHub_projet, illustration_projet
    ...
    public function findAll(): array
    {
        $stmt = $this->pdo->query(
            'SELECT id_projet, titre_projet, texte_projet, date_projet, url_projet, urlGitHub_projet, illustration_projet
             FROM projets ORDER BY date_projet DESC'
        );
    ...
    {
        $stmt = $this->pdo->prepare(
            'SELECT p.id_projet, p.titre_projet, p.texte_projet, p.date_projet, p.url_projet, p.urlGitHub_projet, p.illustration_projet
             FROM projets p
             INNER JOIN projet_competence pc ON pc.id_projet = p.id_projet
             WHERE pc.id_competence = ?
             ORDER BY p.date_projet DESC'
        );
